...
class Dual_s(nn.Module):
    def __init__(self,
        ...
        self.Rgb_backbone = alt_gvt_small(num_classes=0)
        self.Event_backbone = alt_gvt_small(num_classes=0)
        self.embed_dims = embed_dims

        print("embed_dims:", self.embed_dims)

        for channels in self.embed_dims:
            groups = self._get_num_groups(channels)
    ...
    def forward(self, x):
        ...
        event_start = self.rgb_channels
        # event_end = event_start + self.event_channels
        ...
        for stage, (rgb, event, fusion_layer, rgb_norm, event_norm) in enumerate(zip(rgb_features,
                                                                                     event_features,
                                                                                     self.fusion_layers, self.rgb_norms,
                                                                                     self.event_norms)):
            rgb = rgb_norm(rgb)
